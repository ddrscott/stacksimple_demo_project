...
class GuestBooksControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create guest_book" do
    assert_difference('GuestBook.count') do
      post guest_books_url, params: { guest_book: { message: @guest_book.message, name: @guest_book.name } }
    end
  ...
  test "should update guest_book" do
    patch guest_book_url(@guest_book), params: { guest_book: { message: @guest_book.message, name: @guest_book.name } }
